...
const deleteEmail = async (req: Request<{ email_id: string }>, res: Response, next: NextFunction) => {
    try {
        const _delete = await db.query(
            `DELETE FROM emails_trashed WHERE email_id=$1 AND email_trashed_by_id=$2`,
