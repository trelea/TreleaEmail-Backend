...
const archiveEmailMidlleware = async (req: Request<{ email_id: string }, {}, { original_labels: string[] }>, res: Response, next: NextFunction) => {
    ...
    // CHECK IF ITS IN SPAM
    const isSpammed = await db.query(
        `SELECT email_spammed_id FROM emails_spammed WHERE email_id=$1 AND email_spammed_by_id=$2`,
        [req.params.email_id, req?.session?.passport?.user?.email_addr_id]
    );
    if (isSpammed.rowCount !== 0) return res.status(201).json({ msg: 'Invalid Operation' }).end();
